...
#' @examples
#' replacePattern(dir = getwd(), file.type = "R", pattern = "print", replacement = "cat", verbose = T)

replacePattern <- function (dir,
                            file.types = c("r", "R", "Rmd"),
                            search.pattern = NULL, # Pattern for file.type searches
                            pattern,
                            replacement)
{
  if(length(search.pattern) == 0) {
    search.pattern <- paste0("(", paste(file.types, collapse = "|"),
                             ")$")
  }
  ...
  for (doc in rel.docs) {
    ...
    fileFindReplaceVerbose(dir.doc, pattern, replacement)
  }
}
