...
form <- as.formula(paste0("y ~ study_id + ", paste(features, collapse = " + ")))
...
```{r}
models <- c("lr", "l1", "boruta", "svm_no_hp", "svm_hp", "knn_no_hp", "knn_hp")
dlcv_lr <- dlcvLr(folds, rec)
dlcv_l1 <- dlcvL1(folds, rec, features)
dlcv_boruta <- dlcvBoruta(folds, rec, features)
...
dlcv_rf_no_hp <- dlcvRfNoHp(folds, rec)
```

# Combine outputs
```{r}
# Define data frame
obj_names <- ls(pattern = "dlcv_")
obj_models <- unname(sapply(obj_names, function(x) stringr::str_sub(x, 6, nchar(x))))
res <- lapply(obj_names, get)
names(res) <- obj_models

res_metrics <- tibble(model = obj_models) %>% 
  mutate(dlcv = res)

# Reorder data frame
res_metrics <- res_metrics %>% 
  slice(match(models, res_metrics$model))
